import { X, Users, Landmark, Flame as FireIcon, Building2, TreePine, ShieldAlert, Mail, ShoppingBag, Flame, Wind, Droplets, AlertTriangle } from 'lucide-react';
import { useEffect, useRef, useState } from 'react';
import { getHazardInfo } from '../utils/legendLoader';

interface PublicAmenitiesPopupProps {
  name: string;
  category: string;
...
export function PublicAmenitiesPopup({ 
  name, 
  category,
  categoryType,
  subcategory,
  ...
  const getIcon = () => {
    if (!categoryType) {
      console.warn('⚠️ PublicAmenitiesPopup: categoryType is undefined, using fallback icon');
      return Building2;
    }
  ...
  useEffect(() => {
    ...
    if (!parent) {
      console.error('❌ Public Amenities Popup has no parent element!');
      return;
    }
  ...
  return (
    ...
    >
      <div 
        ...
        style={{
          background: 'linear-gradient(to bottom right, rgba(6, 182, 212, 0.05) 0%, rgba(8, 145, 178, 0.05) 100%)'
        }}
      >
        <div className="flex items-start gap-1.5 mb-1.5">
          <div 
            ...
            style={{
              background: 'linear-gradient(to bottom right, #06B6D4 0%, #0891B2 100%)',
        ...
        >
          ...
          />
          <div className="text-[10px] font-semibold" style={{ color: '#06B6D4' }}>
            {category}
