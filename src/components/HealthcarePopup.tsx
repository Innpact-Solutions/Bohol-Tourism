import { X, Building2, Heart, Home, Flame, Wind, Droplets, AlertTriangle } from 'lucide-react';
...
interface HealthcarePopupProps {
  name: string;
  category: string;
  categoryType: string; // Raw category type: 'hospital', 'health_centre', 'nursing_home'
...
export function HealthcarePopup({ 
  name, 
  category,
  categoryType,
  subcategory,
  ...
  const getIcon = () => {
    if (!categoryType) {
      console.warn('⚠️ HealthcarePopup: categoryType is undefined, using fallback icon');
      return Building2;
    }
  ...
  useEffect(() => {
    ...
    if (!parent) {
      console.error('❌ Healthcare Popup has no parent element!');
      return;
    }
  ...
  return (
    ...
    >
      <div 
        ...
        style={{
          background: 'linear-gradient(to bottom right, rgba(14, 165, 233, 0.05) 0%, rgba(2, 132, 199, 0.05) 100%)'
        }}
      >
        <div className="flex items-start gap-1.5 mb-1.5">
          <div 
            ...
            style={{
              background: 'linear-gradient(to bottom right, #0EA5E9 0%, #0284C7 100%)',
